...
![ERD-HousingPrices.PNG](https://github.com/icycake7/main/blob/main/First%20Segment%20Rubric%20-%20Database%20Connection/ERD-HousingPrices.PNG))
...
![script.PNG](https://github.com/icycake7/main/blob/main/First%20Segment%20Rubric%20-%20Database%20Connection/script.PNG)
...
- Presentation: [Presentation](https://docs.google.com/presentation/d/1zto2DDfDRc9m0X1drAQJ9eYMI4gCOl_0CfD11_Fj8GU/edit#slide=id.g1cb5c65843b_0_5)
...
The image shows after plugging in the inputs into the random rorest regression model (number of bedrooms, sqft, number of bathrooms, number of amenities) then you will get a prediction of the price of the monthly rent of the requirements you are looking for. The random forest model rendered an 86% score. Scoring is based off negative mean squared error because we are looking for a positive score. So the more positive the number the better. So with that we can can say we can accurately predict the current house rental prices but not any future housing prices. There are so many other variables that we need to add and more data to be able to confidently predict future house rental prices.
...
The graphs above shows the average prices of monthly rent from each of the 9 cities of our research also along with the population size of each city. You can see that higher populated cities do tend to have higher prices. There are definitely other factors that raise the house prices that our models didn't pick up due to lack of data. The location could be a major factor in some of the higher prices, for example San Francisco had the most expensive housing average while Los Angeles was the most populated. San Francisco could be a more desireable place to live than Los Angeles.
